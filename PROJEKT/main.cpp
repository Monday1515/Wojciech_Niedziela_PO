...
int main()
{
    ...
    // Okno wprowadzania współrzędnych magazynu
    sf::RenderWindow inputWindow(sf::VideoMode(400, 300), "Wprowadz lokalizacje magazynu");

    std::vector<std::string> magazynInputBuffers(2, ""); // Buffery dla współrzędnych X i Y
    int magazynStep = 0;                                 // Indeks aktualnego pola do wprowadzania
    double magazynX = 0.0, magazynY = 0.0;               // Współrzędne magazynu
    ...
    while (inputWindow.isOpen())
    {
        ...
        while (inputWindow.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Enter)
            {
                ...
                else if (magazynStep == 1)
                {
                    ...
                    inputWindow.close(); // Zamknij okno po wprowadzeniu obu wartości
                }
                magazynStep++;
                if (magazynStep > 1)
                    magazynStep = 1;
        ...
        // Rysowanie pól i tekstu wprowadzania
        for (int i = 0; i < 2; ++i)
        {
    ...
    // sf::Font font;
    if (!font.loadFromFile("arial.ttf"))
    { // Upewnij się, że czcionka jest w folderze projektu
        std::cerr << "Failed to load font!" << std::endl;
        return -1;
    }
